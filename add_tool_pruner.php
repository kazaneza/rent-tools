...
/*    foreach ($_POST as $key => $value) {
    ...
    $bladeMaterial = mysqli_real_escape_string($db, $_POST['bladeMaterial']);

    if (!empty($bladeMaterial)) {
        $query = "INSERT INTO Pruner (tool_id, blade_length, blade_material) VALUES ('$tool_id',$bladeLength,'$bladeMaterial')";
    } else{
        $query = "INSERT INTO Pruner (tool_id, blade_length) VALUES ('$tool_id',$bladeLength)";
    }

    $results = mysqli_query($db, $query);
    if ($results == true){
        array_push($query_msg,"Writing tool info to Pruner");
    } else{
        array_push($error_msg, "Query Error: Unable write to Pruner...". $query);
...
<div id="main_container"><?php include("lib/menuNavClerk.php"); ?>
    <div class="center_content">
        <div class="title_name">Add Pruner Type & Subtype Attributes</div>
        <form id="pruner" name="pruner" method="post" action="add_tool_pruner.php">
