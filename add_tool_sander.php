...
/*    foreach ($_POST as $key => $value) {
    ...
    $dustbag = mysqli_real_escape_string($db, $_POST['dustbag']); //attribute of Sander
    ...
    $dustbag = (bool)$dustbag;
    $query = "INSERT INTO Sander (tool_id,dust_bag) VALUES ($tool_id,$dustbag)";
    $results = mysqli_query($db, $query);
    if ($results == true){
        array_push($query_msg,"Writing attributes to Sander");
    } else{
        array_push($error_msg, "Query Error: Unable write to Sander...". $query);
        array_push($error_msg,  'Error# '. mysqli_errno($db) . ": " . mysqli_error($db));
    }
...
<head>
    ...
    <title>AddTool Sander:Sub-Type Attributes</title>
...
<div id="main_container"><?php include("lib/menuNavClerk.php"); ?>
    <div class="center_content">
        ...
        <form id="sander" name="sander" method="post" action="add_tool_sander.php">
        ...
        <?php if ($_SESSION['power'] == 'cordless'): ?>
            ...
            <hr>
        <?php endif; ?>
        <h3 style="margin-left:10px">Add Sander Attributes</h3>
            ...
            <table style="margin-left:10px">
                <tr><td class="heading">Dust Bag:</td><td><select name="dustbag" required><option value="true">True</option>
        ...
        <hr>
        <form action="add_sander_accessories.php">
